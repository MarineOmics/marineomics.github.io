...
Deciding on a sequencing approach for a new project can be challenging. Your choice depends primarily on the questions of interest, but also constraints set by funding, existing genomic resources, sample size, genome characteristics and expertise in the research group. The main categories of population genomic sequencing approaches include (1) High coverage individual whole-genome resequencing (hcWGS) and (2) Low coverage individual WGS (lcWGS), (3).

**Coverage depth** refers to the average number of sequencing reads per region of the genome (distinct from coverage breadth, see [@sims2014sequencing]). With more sequencing reads at a region (higher coverage), errors that arise during sequencing have a smaller impact on accurately identifying variable sites (termed “genotyping”) [@Fuentes-Pardo2017-nu].

### 1) High coverage individual whole-genome resequencing (hcWGS)  {.unlisted .unnumbered}
**Whole-genome resequencing (WGS)** is where whole genomes of multiple individuals are sequenced and mapped to an assembled reference genome in order to compare variable sites between individuals or populations. This approach is considered the is the 'gold standard' for DNA resequencing because it includes all variant types (i.evSNVs, indels, structural variants and CNVs) [@sims2014sequencing].

### 2) Low coverage individual WGS (lcWGS)  {.unlisted .unnumbered}
Low coverage whole genome sequencing is where whole genomes of many individuals are sequenced, but depth of coverage is so low (sequencing depth < 3) that individual genotypes cannot be confidently assigned [@korneliussen2014angsd]. Instead of assigning specific, “hard called" genotypes, lcWGS data requires probabilistic analyses that account for uncertainty about the true genotype and can incorporate uncertainty across all sequenced samples.
