...
Initial publication year: 2023 <br>
...
ls -d -1 $bamdir/*.* > $bamdir/f2.paths.list

# merge bam files
samtools merge $bamdir/f2.merged.bam -b $bamdir/f2.paths.list

# index bam files
samtools index $bamdir/f2.merged.bam
```
...
#### Identify useful SNPs 
  ...
  * In this case, I only had sequence data for one of the parents (Pa), which narrows the list of useful sites:
    ...
    * Now we'll call genotypes for both the F1 and Pa at these sites using a combination of [samtools mpileup](http://www.htslib.org/doc/samtools-mpileup.html), [bcftools call](https://samtools.github.io/bcftools/bcftools.html#call), and [bcftools query](https://samtools.github.io/bcftools/bcftools.html#query). This is a simple text file with 4 columns: 
      * chromosome, position, reference allele, and the alternate allele:
      ```{bash, eval = F}
      ...
      ```
    * Finally, we'll print a file with the sites, the Pa allele, and the Pb allele
    ```{bash, eval = F}
    awk ' { if ( $5 ~ /0\/0/ ) print $1 "\t" $2 "\t" $3; else print $1 "\t" $3 "\t" $2; } ' f1.pa.genotypes > pos.pa.pb.txt    
    ```
