...
Files are then deduplicated if they are whole genome bisulfite treated samples.
...
This will create deduplicated.bismark.cov.gz, uncompressed is in this format. And while a bedGraph file is generated, in this workflow a bedGraph will be generated later that uses information from merged strands (as opposed to this output).
...
A genome-wide cytosine report ins generated, with this code as an example:
...
#Generating files for downstream analysis

From here it could be useful to do some simple file reorganizaion to obtain bedgraph or tab files for down stream analysis. 
...
It is often useful to sort files for downstream analysis (eg methylkit, IGV)
